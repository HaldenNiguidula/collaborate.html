...
<body>
  ...
  <script>
    ...
    /**
     * Updates displayed Cash Sales and GCash Sales amounts.
     * Since localStorage transactions are removed, these remain zero.
     */
    function updateSalesAmounts() {
      ...
      // No persistence data, so set zero
      gcashEl.textContent = formatter.format(0);
      cashEl.textContent = formatter.format(0);
    }

    // In-memory arrays for entries instead of localStorage
    const cashAddedEntries = [];
    const cashExpenseEntries = [];
     ...
     * @param {string} type - 'added' or 'expense' to distinguish entry list
    ...
    function createEntryElement(entry, type, index) {
      ...
      // Remove handler removes entry from in-memory array and rerenders
    ...
    /**
     * Remove entry by type and index, then rerender entries.
     */
    function removeEntry(type, index) {
      if (type === 'added' && index >= 0 && index < cashAddedEntries.length) {
        cashAddedEntries.splice(index, 1);
        renderEntries();
      } else if (type === 'expense' && index >= 0 && index < cashExpenseEntries.length) {
        cashExpenseEntries.splice(index, 1);
        renderEntries();
      }
    }
    ...
    /**
     * Form submit handler
     * Validates input, saves new entry in-memory, updates UI, then closes modal
     */
    entryForm.addEventListener('submit', (e) => {
      ...
      const entry = { amount: amount.toFixed(2), details };

      if (currentEntryType === 'added') {
        cashAddedEntries.push(entry);
      } else if (currentEntryType === 'expense') {
        cashExpenseEntries.push(entry);
      }
      renderEntries();
      hideEntryModal();
    });
